...
class SuperController extends Controller
{
    ...
    public function tradeManager($type = 0)
    {
        ...
        $data = array();
        foreach ($parkrecord_array as $parkrecord) {
            $car_user=$Car->relation(true)->find($parkrecord['car_id']);
            $park_user=$Park->relation(true)->find($parkrecord['park_id']);
            array_push($data, array(
                'carNo' => $parkrecord['Car']['no'],
                'name' => $car_user['User'][0]['name'],
                'telphone' => $car_user['User'][0]['phone'],
                'parking' => $parkrecord['Park']['name'],
                'position' => '5-03',
                'regTelphone' => $park_user['User']['phone'],
                'inTime' => date('Y-m-d H:i:s', $parkrecord['start_time']),
                'outTime' => date('Y-m-d H:i:s', $parkrecord['end_time']),
                'sumConsume' => $parkrecord['money']
            ));
        }
